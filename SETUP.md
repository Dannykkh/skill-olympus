...
통합 설치는 전역 규칙과 자동 대화 저장 훅도 설치합니다. 실행 전에 [전역 규칙 변경·개인 수정·저장 중지·제거 안내](docs/global-agent-rules.md)를 확인하세요. 재설치하면 관리 블록 안의 수정은 교체되고, 마커 밖 개인 규칙은 보존됩니다. Grok의 전용 관리 규칙 파일은 전체 교체됩니다.
