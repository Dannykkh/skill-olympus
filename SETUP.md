...
공개 추적 스킬 소스 99개는 기본 allowlist 합집합 17개(사용자 진입점 하네스 11개 + 런타임 어댑터 6개)와 source-only 내부·선택 모듈 82개로 나뉩니다. 호환되지 않는 어댑터를 제외하면 Claude는 96개(활성 14 + source-only 82), Codex/Gemini는 각각 95개(활성 13 + source-only 82)입니다. Grok 설치 표면은 Claude의 활성 14개를 공유합니다. 내부 전용 `deploymonitor`는 로컬에만 있고 공개 배포에서 제외됩니다. 활성 하네스는 필요한 하위 모듈을 카탈로그에서 직접 읽고, 나머지 저빈도 가이드·변환기도 같은 source-only 경로에서 명시 요청할 수 있습니다.
