...
각 스킬은 역할에 맞는 그리스 신화 캐릭터(또는 야구 포지션)의 이름을 가지고 있습니다.

| 스킬 | 이름 | 유래 | 역할 |
|------|------|------|------|
| `/zephermine` | 젭마인 | — | **설계사** — 26단계 심층 인터뷰, 스펙 생성, 5인 전문가 팀 리뷰 |
| `/zeus` | 제우스 | 신들의 왕 | **총사령관** — 제로 인터랙션 풀 파이프라인, 모든 것을 통솔 |
| `/agent-team` | 대니즈팀 | — | **시공팀** — 웨이브 그룹 병렬 구현 |
| `/workpm` | 다이달로스 | 장인의 대가 | **현장감독** — 설계 없이 바로 구현하는 PM, 직접 손대는 장인 |
| `/argos` | 아르고스 | 100개의 눈을 가진 감시자 | **감리관** — 준공검사, 설계 대비 구현 검증 |
| `/qpassenger` | 큐패신저 | — | **QA 테스터** — Playwright E2E 테스트 + fix-until-pass 루프 |
...
| 버전 | 날짜 | 핵심 |
|------|------|------|
| [v1.9.0](https://github.com/Dannykkh/claude-code-agent-customizations/releases/tag/v1.9.0) | 2026-03-24 | 아테나 CEO 코칭 + 파이프라인 확장 |
