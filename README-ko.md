...
| 버전 | 날짜 | 핵심 |
|------|------|------|
| **[v5.4.0](https://github.com/Dannykkh/skill-olympus/releases/tag/v5.4.0)** | **2026-08-25** | **진입점 정책 재정의 + seo-audit 2축 점수** — 활성 판정을 신화 이름 기반에서 네 조건(고유 slash·완결 산출물·네이티브 비중복·호출 빈도)으로 재정의; "참조됨은 강등 근거 아님" 명문화; allowlist 17→24로 `api-tester`·`explain`·`hestia`·`ko-en-translator`·`release-notes`·`seo-audit`·`video-maker` 7종 승격, 하네스 내부 모듈 3종은 source-only 유지; 활성 Claude/Grok 21·Codex/Gemini 20; seo-audit v3.0.0 검색/AI 가시성 2축 점수 + `na` 재정규화 + P0 40점 상한 + 한국어 팩트 밀도 계측기; `llms.txt` 배점 0·`FAQPage` P1→P2; 문서 개수 표기를 공개 100 기준으로 통일 |
