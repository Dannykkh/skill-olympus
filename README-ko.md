...
| `/agent-team` / `/poseidon` | 포세이돈 | — | **시공팀** — 웨이브 그룹 병렬 구현. 바다의 신이 파도를 다스리듯 섹션 의존성 그래프를 Wave 단위로 정렬 |
| `/workpm` | 다이달로스 | 장인의 대가 | **현장감독** — 설계 없이 바로 구현하는 PM, 직접 손대는 장인 |
| `/argos` | 아르고스 | 100개의 눈을 가진 감시자 | **감리관** — 준공검사, 설계 대비 구현 검증 |
| `/minos` | 미노스 | — | **QA 테스터** — Playwright E2E 테스트 + fix-until-pass 루프 |
| `/clio` | 클리오 | 야구 역사의 뮤즈 | **역사의 뮤즈** — 파이프라인 완료 후 흐름도 + 문서 산출물 |
