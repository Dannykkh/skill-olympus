...
- 구현은 YAGNI → 코드베이스 재사용 → 표준 라이브러리 → 네이티브 기능 → 기존 의존성 → 최소 신규 코드 순서로 올라가고, 의도적으로 단순화한 지름길에는 `// minimal: <상한> — <업그레이드 시점>` 주석을 남겨 `/hestia`가 부채 장부로 수확합니다.
...
기존 Codex 설치기는 `config.toml`의 `notify`를 구성하고 `tui.notifications=false`, `tui.animations=true`, `tui.whimsy=false`도 설정합니다. save-turn이 이미 연결된 체인은 유지하지만 일부 데스크톱·IDE 알림 전용 체인은 교체합니다. 알림을 직접 구성했다면 [설정 변경·제거 시 동작](docs/global-agent-rules.md)을 확인하세요.
...
Olympus는 `install.bat` 또는 `install.sh`의 Codex 설치 단계에서 **스피너 등 일반 터미널 애니메이션은 켜고, 별 같은 장식 효과는 전역으로 끕니다**. 설정 파일은 Windows의 `%USERPROFILE%\.codex\config.toml`, macOS/Linux의 `~/.codex/config.toml`입니다. `CODEX_HOME`을 지정했다면 그 디렉터리의 `config.toml`을 수정합니다.
...
**별 효과를 다시 켜려면 `animations=true`를 유지하고 `whimsy=true`로 바꾼 뒤 Codex를 재시작하세요.** 설치·재설치 시 `animations=true`, `whimsy=false`를 적용하며, 이전 설치에서 `animations=false`였어도 `true`로 갱신합니다. Olympus 제거 후에도 이 설정은 유지됩니다. [공식 설정 스키마](https://developers.openai.com/codex/config-schema.json) · [설치 설정 상세](docs/global-agent-rules.md)
...
Codex 동기화는 `${CODEX_HOME:-~/.codex}/skills`와 `~/.agents/skills`의 직속 스킬 폴더도
대조합니다. frontmatter 이름과 전체 파일 구성이 같으면 Codex 전용본을 활성으로 두고,
공용본만 `config.toml`에서 비활성화합니다. 두 원본은 모두 보존하며, 내용이 다르면 차이를
안내하고 사용자가 명시한 활성·비활성 설정은 유지합니다. 이후 동기화에서 내용이 달라지거나
한쪽이 없어지면 설치기가 추가한 비활성화를 해제합니다. 제거 시에도 수정되지 않은 설치기
관리 설정 블록만 회수합니다. 변경 전 설정은 Codex 홈의 `.olympus/backups/`에 백업합니다.
링크된 리소스나 지원하지 않는 설정 형식은 경고 후 수동 검토 대상으로 남깁니다.
중첩 스킬 폴더와 플러그인 번들은 이 검사에서 제외합니다. 설정 변경 후 Codex를 재시작하세요.
서로 다른 스킬이 많으면 설명 길이 제한 경고는 남을 수 있습니다.
...
- **도메인 용어:** 질문을 시스템 용어가 아니라 **그 업계 용어로** 합니다 — "공용 화면 권한차등인가요?"가 아니라 "상담원이 마케팅팀과 같은 쿠폰 화면을 보되 발행 버튼만 안 보이면 되나요?". 어휘는 사용자가 쓴 원문 → 리서치 → 글로벌 도메인 사전(읽기 전용) 순으로 확보합니다.
...
`gs-certification`은 한국의 GS(Good Software) 인증을 준비하는 **선택 실행 스킬**입니다.
GS 준비 요청에서만 1·2등급을 구분해 제품·문서·시험·결함·회귀 증거를 연결하며, 일반 개발에는 자동 적용하지 않습니다.

### 설치와 사용

```powershell
# Windows — Claude, Codex, Antigravity, Grok 설치/동기화
install.bat --all
```
...
기본 설치는 source-only입니다. 네 CLI의 카탈로그에서 정확한 SKILL.md를 찾아 읽으며,
Grok은 Claude의 공용 설치 경로를 사용합니다. 기본 slash 등록을 가정하지 않습니다.
제품 프로젝트에서 다음처럼 요청하세요.

```text
GS 인증 1등급 사전점검해줘
GS 인증 2등급 검사해줘. 점검만 하고 제품 코드는 수정하지 마.
GS 인증 1등급 대비해서 발견한 문제를 수정하고 회귀시험까지 해줘.
```

등급을 생략하면 먼저 확인합니다. 1등급은 품질평가, 2등급은 품질평가와 업무적합성을 별도로 관리하며,
기준과 결과는 기관·등급·제품 빌드·시험 환경별로 구분합니다. 점검 요청만으로 제품 수정·외부 제출을 수행하지 않습니다.

### ISO 기반 절차와 담당

[TTA 공식 안내](https://cs.tta.or.kr/tta/introduce/introCont.do?tabMode=cont&tnc_cls_no=T000127&tnc_lab=T000003&up_tnc_cls_no=T000020)에 따라
ISO/IEC 25023(품질 측정), 25041(품질 평가 지침), 25051(제품·문서 요구와 시험)을 기준 확인의 출발점으로 삼습니다.
실제 항목·측정법·기준치는 **신청기관·등급·제품 유형의 현행 시험평가기준**으로 확정합니다.

**기준·범위 확정 → 측정/시험 설계 → 실행 → 결함 보완·회귀 → 문서·증거 감리 → 준비도 보고**

| 담당 | GS 요청에서의 역할 |
|---|---|
| GS 스킬 | 기준 추적표, 등급별 실행 계획, 증거·결함 장부와 최종 준비도 |
| 코드리뷰어 | 서버 인가·입력값·업무 조건 및 실제 assertion 검토 |
| api-tester·미노스 | 정상/오류/경계값·직접 API 요청과 회귀시험 |
| 클리오 | 제품설명서·사용자취급설명서 대조와 절차 재현 |
| 아르고스 | 기준·측정·실행 결과·문서·결함/회귀의 연결 감리 |

필요한 모듈만 실행합니다. 헤스티아의 자동 정리나 모든 스킬의 일괄 실행을 강제하지 않습니다.
실행기는 검토한 프로젝트 테스트 명령을 실행해 시각·종료 코드·출력 해시를 수집합니다.
`EVIDENCE_COMPLETE`는 자동 증거 수집 완료이며, 아르고스 검토 전 READY로 간주하지 않습니다.

### 결과와 검증 범위

산출물은 `docs/gs-certification/grade-{1|2}/{run-id}/`에 기준표·시험 계획·실행 증거·문서 검토·결함·감리 결과로 저장합니다.
준비도는 READY / NEEDS_WORK / INCOMPLETE로 보고하며 **공식 인증 합격이나 공인 시험성적서를 대신하지 않습니다**.
기관 기준·장비·계정·실행 증거가 부족하면 미검증으로 남깁니다.

샘플 검증은 실제 loopback HTTP에서 권한 우회·비정상 입력·거부 후 저장/이벤트 변경을 탐지하고,
등급 누락·2등급 업무 결과·시간 초과·증거 덮어쓰기 방지를 확인합니다. 실제 제품 전체나 ISO 전문 전 조항을 검증한 것은 아닙니다.
장애 복구·재시작 후 영속 데이터 보존·DB 복원은 **프로젝트별 시험을 추가해야 하며 샘플에서 검증하지 않았습니다**.

[스킬](skills/gs-certification/SKILL.md) · [실행 프로토콜](skills/gs-certification/references/execution-protocol.md) ·
[기준 출처](skills/gs-certification/references/sources.md)
