...
> 루프는 "계속하려는 의지"가 아니라 "멈출 수 없는 구조"로 유지됩니다.
> 다섯 가지 사인(死因)에 다섯 가지 구조적 처방: 심장박동은 기계에, 상태는 컨텍스트 밖에,
> 막힘은 정지가 아니라 주차로, 유휴는 정의된 폴백으로, 사람은 결재 가능한 브리프로만.
> Peter Steinberger가 공개한 maintainer-orchestrator 루프에서 증류한 패턴입니다.

<p align="center">
  <img src="docs/assets/chronos-loop-programming.svg" alt="크로노스 루프 프로그래밍 사이클" width="860">
</p>

루프 상태는 컨텍스트 윈도우가 아니라 감사 로그에 있습니다. 매 사이클은 READ로 재진입하고, 객관적 검증을 실행하며, 유효한 막힘은 결재 가능한 브리프로 주차하고, 훅은 거짓 완료 선언을 거부합니다.

- **크로노스 주차(PARK) 규칙** — 막힌 이슈 하나가 루프 전체를 멈추지 못합니다. 유효한 주차 사유 4가지(검증 3회 실패 / 권한 경계 / 외부 접근 부재 / 제품 결정); 사유 없는 "막힘" 선언은 회피로 간주; 주차 전에 갈 수 있는 데까지(재현, 원인 분석, 권한 안의 수정) 가야 함
- **Owner Decision Brief** — 주차된 이슈는 날것의 질문이 아니라 결재 가능한 상태로 보고: 무엇 / 왜 지금 / 증거 / 트레이드오프 / **추천(의무 — 기술 분석을 사용자에게 떠넘기지 않음)** / 정확한 선택지. 사용자의 일은 4지선다가 됩니다: 추천대로 승인, 반려, 정확히 하나의 접근 권한 부여, 문서화된 대안 중 택일
- **재진입 규약(READ 단계)** — 매 사이클 FIND 전에 `docs/chronos/chronos-log.md`를 재독; 모델의 기억과 로그가 어긋나면 **로그가 이깁니다**. 새 세션이 감사 로그만으로 루프를 이어받음 — 루프의 상태는 컨텍스트 윈도우가 아니라 파일에 있음
- **데드락 가드** — goal 목표문에 주차 조항 포함: 주차만 남으면 Brief를 담은 `Chronos Complete` 보고로 종료. 거짓 `<promise>` 출력 금지 — 그리고 훅이 더 이상 보상하지 않음: loop-stop/continue-loop의 "`<promise>` 태그만 있으면 완료" 분기 제거(불일치 promise는 종료 대신 재투입), 여러 줄 promise 매칭 보강, 재투입 메시지에 주차 규칙 주입. loop-stop.ps1 기능 테스트 4/4 통과(불일치 거부 / 정확 일치 / 주차만 남은 완료 / 마커 없음 재투입); .sh는 동일 로직 + 문법 검증
- **제우스 결정 장부(Decision Ledger)** — `[ZEUS-AUTO:taste]` 결정에 근거 + 기각한 대안 + **되돌리는 법** 기록; zero-interaction은 유지 — 결재를 사전이 아닌 사후로 옮기고, 되돌리기 쉬운 기본값을 우선
- **codex-mnemo 설치기 수정** — notify 래퍼 판정 순서 교정: save-turn을 체인하는 래퍼는 IDE 알림 제거 휴리스틱보다 먼저 보존(갱신). Codex의 단일 notify 슬롯을 공유하는 외부 도구 통합이 재설치로 조용히 끊기지 않음
...
| 버전 | 날짜 | 핵심 |
|------|------|------|
| **[v4.8.0](https://github.com/Dannykkh/skill-olympus/releases/tag/v4.8.0)** | **2026-06-13** | **루프 프로그래밍: 주차, 브리프, 재진입** — 크로노스 주차 규칙(사유 4가지; 사유 없는 막힘 선언 = 회피) + Owner Decision Brief(결재 가능 에스컬레이션, 추천 의무, 4지선다) + 재진입 규약(로그가 기억을 이김; 새 세션이 감사 로그로 재개); 주차만 남은 큐의 데드락 가드(거짓 `<promise>` 금지; 훅의 불일치 태그 보상 제거 — ps1 4/4 테스트, 여러 줄 매칭 보강, 재투입 메시지에 주차 규칙); 제우스 결정 장부(근거 + 대안 + 되돌리는 법, 사후 결재); codex-mnemo notify 판정 순서 수정(save-turn 체인 래퍼 보존) |
