...
| Periodically, or when in doubt | 13 checks: index budget and links, entry metadata, oversized files, lifecycle links, distill baseline, absolute paths inside records, misclassified observations, stale code anchors, handoffs, history recoverability, record coverage. `--fix` repairs only three mechanical things — the distill baseline, absolute in-project paths in handoff headers / tool logs / observation logs rewritten relative to the root, and tag-name lifecycle links rewritten to `[[NNN-slug]]` when exactly one entry matches | `mnemo_doctor.py [--fix]` | yes |
