...
| Periodically — and automatically at the first handoff after 30 days | 16 checks; the three added here are entry evidence (links that do not open, anchors living only in prose), decision lineage (`SUPERSEDED` without a reason, a `CURRENT` entry resting on a superseded one) and unattached conversations (days no trunk reaches, ranked by decision vocabulary). `--chart` records the visit so the next one reports only what changed; `--fix` still repairs only the same three mechanical things, and entry bodies are touched only by the separate `--promote-structure` | `mnemo_doctor.py [--chart] [--fix]` | yes |
