...
- **Role interview:** step 6 asks which principals sign in, what each one does, what it must never do, and whether it needs a separate screen or only different permissions on a shared one. The screen answer is escalated to a critical question. A full-access superadmin, role inheritance, and owner-level access are applied as defaults without asking.
...
- **Modules and harness:** sections are split by feature, not by layer, so one feature owns its data, API, screen and tests and can be opened and fixed on its own. Layer splits (`api-layer`, `frontend`) scatter a feature and are rejected unless an existing codebase forces them. Each section declares a Module Contract — Provides, Consumes, Owns, Composition Point — and `sections/index.md` records the runtime harness: the existing router / DI container / plugin registry the modules register into. Empty Provides means the section is internal work and gets merged; overlapping Owns means a parallel-build conflict and gets raised to the shared foundation.
- **Domain wording:** questions are asked in the industry's own vocabulary, not system terms — "does the support agent see the same coupon screen minus the issue button?" rather than "shared screen with permission variance?". Vocabulary comes from the user's own wording first, then research, then the global domain dictionary (read-only).
...
- **Per-role screens:** when a Zephermine plan is present, Aphrodite reads its Role Inventory and RBAC matrix instead of inventing role names. Roles marked "shared screen" get a role-variant grid (element x role, valued visible / hidden / disabled / read-only); roles marked "separate screen" get their own layout blueprint. Hidden and disabled are kept distinct — hidden conceals that the permission exists, disabled admits it and blocks. The contract validator requires the variant grid whenever roles are declared.
...
| End of session | The agent supplies the verified request and evidence with `--origin` and `--origin-source` (both required together); absent evidence stays TODO, and previous handoffs are linked only with `--continues-from`. The scaffold fills `Files Modified` from the observation log (only that session's when the origin evidence names `session <uuid>`), and offers the existing entries that rest on the files you touched as supersede candidates; it also rebuilds the anchor index for every CLI, and in projects with a TermSnap component map it reports this session's unowned files. Then validate: a supersede target that does not exist is a gate; decisions that never reached memory, and entries this session touched whose number never reached its tag line (`arch:NNN`, `learned:NNN`, `gotcha:NNN`, colon form only), and component-map findings left neither assigned nor deferred, are warnings | `create_handoff.py` → `validate_handoff.py` | yes |
