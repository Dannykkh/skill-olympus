...
### 후보 생성 (매트릭스 작성 전 필수 선행)

트레이드오프 매트릭스를 채우기 전에, 서로 뚜렷이 구별되는 후보 아키텍처 2~3개를 먼저 생성합니다(예: 모놀리식 vs 모듈러 모놀리식 vs 마이크로서비스). 각 후보마다 핵심 접근·전제·주요 리스크를 한 줄로 기술합니다.

> 채점 없이 후보만 나열 금지. 후보를 만들었으면 반드시 아래 매트릭스의 가중 기준으로 점수화하여 비교합니다.
...
> 선택지 표는 구별되는 후보 2개 이상으로 채웁니다. 채점 없이 후보만 나열 금지 — 트레이드오프 매트릭스 가중 기준으로 비교한 뒤 결정을 적습니다.
