...
주장/수치/날짜를 포함한 문서(제안서, 초록, 그랜트, 보고서)는 최종화 전 반드시 거칠 것:

- **출처 확인 없이 사실 주장 금지**: 모든 수치/날짜/인용/고유명사를 원본 자료에 대조 검증한다. 모델은 외부 근거 없이 사실을 자가 교정할 수 없음.
- 검증 불가능한 항목은 삭제하거나 `[출처 확인 필요]`로 표시해 작성자에게 넘긴다 — 추정으로 메우지 말 것.
- **페르소나/역할은 톤과 문체를 정할 뿐, 사실 정확성을 보장하지 않는다.** 전문가 톤이 곧 정확한 사실은 아님.
