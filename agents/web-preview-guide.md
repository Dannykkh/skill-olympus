...
프리뷰 모드에서 생성하는 웹사이트에 GSAP 애니메이션을 기본 탑재합니다.
...
**애니메이션 토큰 CSS 변수** (`globals.css` 또는 디자인 토큰 파일):
```css
:root {
  --motion-duration-fast: 0.3s;
  --motion-duration-normal: 0.6s;
  --motion-duration-slow: 1s;
  --motion-ease-out: cubic-bezier(0.16, 1, 0.3, 1);    /* gsap: "power3.out" */
  --motion-ease-in-out: cubic-bezier(0.65, 0, 0.35, 1); /* gsap: "power2.inOut" */
  --motion-stagger: 0.08;
  --motion-slide-distance: 40px;
}
```
...
useEffect(() => {
  const ctx = gsap.context(() => {
    // 모든 GSAP 애니메이션을 이 안에서 생성
    gsap.from('.hero-title', { y: 40, opacity: 0, duration: 0.8, ease: 'power3.out' })
  }, containerRef) // scope를 컨테이너 ref로 제한

  return () => ctx.revert() // 언마운트 시 모든 애니메이션 정리
}, [])
```

**접근성 처리** (`prefers-reduced-motion`):
```ts
// 앱 진입점에서 한 번만 설정
const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches
if (prefersReducedMotion) {
  gsap.globalTimeline.timeScale(20) // 모션 즉시 완료
...
|#|패턴|GSAP 코드|용도|
|---|---|---|---|
|1|Fade-in on Scroll|`gsap.from(el, { y: 40, autoAlpha: 0, scrollTrigger: { trigger: el, start: 'top 85%' } })`|섹션 진입 애니메이션|
|2|Hero Stagger|`gsap.from('.hero *', { y: 30, autoAlpha: 0, stagger: 0.08, ease: 'power3.out' })`|히어로 순차 등장|
|3|Parallax|`gsap.to('.bg', { yPercent: -20, scrollTrigger: { scrub: true } })`|배경 패럴랙스|
|4|Card Hover|`gsap.to(card, { scale: 1.03, boxShadow: '0 20px 40px rgba(0,0,0,0.15)', duration: 0.3 })`|카드 호버 효과|
|5|Flip Layout|`const state = Flip.getState(items); /* DOM 변경 */ Flip.from(state, { duration: 0.5, ease: 'power2.inOut' })`|레이아웃 전환|
|6|Text Reveal|`gsap.to('.text', { text: { value: '최종 텍스트' }, duration: 1, ease: 'none' })`|타이핑 효과|

**참조 구현**: `site/src/pages/LandingPage.tsx`
...
화면 완성 후 배포 전 성능 측정 + 품질 검증.

### Step 1. 성능 측정 (PreviewPanel CDP 내장)

PreviewPanel에 이미 CDP 연결이 있으므로 **프로그램이 직접 측정**합니다.
Lighthouse CLI는 별도 Chrome을 띄워 dev server 포트 충돌이 발생하므로 사용하지 않습니다.

**측정 방법** (PreviewPanel CDP):
```
Performance.getMetrics()           → FCP, LCP, 레이아웃 횟수, 스크립트 실행 시간
Runtime.evaluate(web-vitals)       → CLS, FID/INP, TTFB
Performance.getResourceTiming()    → 리소스별 로드 시간 (이미지, JS, CSS)
```
...
|LCP|< 2.5s|최대 콘텐츠 페인트|
|CLS|< 0.1|누적 레이아웃 이동|
|INP|< 200ms|다음 페인트까지 인터랙션 시간|
|FCP|< 1.8s|첫 콘텐츠 페인트|
|TTFB|< 800ms|첫 바이트 수신 시간|

**자동 수정 루프**: 페이지 로드 시 자동 측정 → 기준 미달 시 AI CLI에 결과 전달 → 코드 수정 제안.

```
페이지 로드 → CDP 자동 측정 → 기준 미달 항목 추출 → AI CLI에 전달 → 수정 → 재측정
```

> 콘솔 에러 자동 감지 → AI 전달과 동일한 패턴. 성능 메트릭도 같은 경로로 흐른다.
...
**기준**:
- 초기 로드 JS: 200KB 미만 (gzip)
- 개별 chunk: 100KB 미만
- 미사용 코드 비율: 20% 미만

### Step 3. 반응형 자동 검증 (PreviewPanel CDP 내장)

PreviewPanel이 CDP `Emulation.setDeviceMetricsOverride`로 뷰포트를 전환하며 자동 검증합니다.

```
CDP 호출 흐름:
Emulation.setDeviceMetricsOverride(width: 375, height: 812, deviceScaleFactor: 3)
→ Page.captureScreenshot → AI에게 전달 → 레이아웃 깨짐 감지
→ 다음 뷰포트로 반복
```
...
|기기|너비|확인 포인트|
|---|---|---|
|Mobile S|375px|터치 타겟 44px, 텍스트 가독성, 햄버거 메뉴|
|Mobile L|428px|가로 스크롤 없음|
|Tablet|768px|2열 레이아웃 전환|
|Desktop|1024px|사이드바/네비게이션 전환|
|Desktop L|1440px|max-width 제한, 여백 균형|

**UI**: PreviewPanel 하단에 뷰포트 전환 버튼 `[375] [768] [1024] [1440]` 표시.

### Step 4. SEO & 메타태그 (랜딩/홈페이지 프로젝트)

홈페이지, 랜딩 페이지, 마케팅 사이트 프로젝트에서는 SEO 기본 요소를 자동 생성합니다.

**필수 생성 항목**:

|항목|내용|
|---|---|
|`<title>`|페이지별 고유 타이틀 (60자 이내)|
|`<meta name="description">`|페이지 설명 (155자 이내)|
|`og:title`, `og:description`, `og:image`|소셜 공유용 OG 태그|
|`robots.txt`|크롤러 허용/차단 규칙|
|`sitemap.xml`|전체 페이지 URL 목록|
|`canonical`|중복 URL 방지|
|구조화 데이터|`JSON-LD` (Organization, Product, FAQ 등)|

**적용 조건**: SPA 앱이나 관리자 대시보드에는 적용하지 않음. 공개 웹사이트/랜딩 페이지에만.
...
|4|라이트모드 텍스트 대비 4.5:1|Lighthouse Accessibility|
|5|키보드 focus 상태 가시적|Tab 키로 확인|
|6|prefers-reduced-motion 존중|접근성 설정 변경 후 확인|
|7|이미지 최적화|WebP/AVIF 변환, lazy loading, width/height 명시|
|8|폰트 로딩|font-display: swap, preload 적용|
