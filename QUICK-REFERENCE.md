...
| `skills/zephermine/` | 저장형 설계 자동화 — Antigravity의 가벼운 `/plan`·`/grill-me`보다 큰 spec(시스템 역할 표)→API/DB→역할 레인 flow→RBAC·QA→기능 모듈 sections(Module Contract + 하네스) 묶음 |
...
| `skills/video-maker/` | 영상 요청 진입점 — Remotion(React/TSX)·HyperFrames(HTML/CSS/GSAP) 중 엔진 하나로 스크립트→음성(TTS)→컴포지션→자막→렌더, TTS 공급자 선택, 공통 asset·render QA |
