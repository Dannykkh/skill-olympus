...
|-------|--------|--------|----------|
| **설계** | `/zephermine` | 스펙(시스템 역할 표), 기능 모듈 섹션(Module Contract) + 하네스, QA 시나리오, API 명세(허용 역할), DB 스키마, 역할 레인 도면, RBAC 매트릭스 | **무엇**을 만드는가? |
| **디자인** | `/aphrodite` | DESIGN.md, Experience Contract, 레이아웃 청사진, 역할별 화면 변형표 | **어떻게** 보이는가? |
...
| 종류 | 이름 | 역할 |
|------|------|------|
| **스킬** | zephermine | 26단계 워크플로우 (리서치→인터뷰→스펙+시스템 역할 표→6전문가 검토→계획→DB/API/도면→섹션→운영·QA→역할명 정합성 검증) |
