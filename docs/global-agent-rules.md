...
프로젝트 저장 경계의 정본은 [Mnemo 공통 규약](../skills/mnemo/references/project-storage.md)이다.
대화 훅과 핸드오프·복구 도구가 같은 판별기를 사용하며, 일반 `cwd`만으로 새 저장 루트를
만들지 않는다. 비-Git 프로젝트는 실제 workspace를 지정하거나 루트에 빈 `.mnemo-root`를
만든 후 사용한다. 각 CLI의 설치된 Mnemo 스킬에는 같은 핸드오프 도구와 규약이 포함된다.
