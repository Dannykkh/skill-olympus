...
3. **Verify**: 테스트/체크로 검증한다. 실패 시 최대 3회 재시도. 3회 실패 → 능력 1회 상향(추론 effort 또는 더 강한 모델; 둘 다 불가하면 실패 출력을 증거로 접근 1회 재설계) → 그래도 실패 → 사유를 적어 주차(Park)하고 다음 이슈로. 같은 방식 재시도만으로 주차 금지(에스컬레이션은 이슈당 1회).
