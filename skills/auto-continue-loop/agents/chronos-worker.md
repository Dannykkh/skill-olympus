...
1. **Find**: **도구 신호에서 시작한다(외부 근거 우선)** — 모델 직관 전에 가용한 정적 신호(테스트·`tsc --noEmit`/타입체크·lint·빌드 에러; 보안 스코프면 보안 스캔)를 먼저 모아 구체적 실패 중 최우선 1개를 고른다. 도구 신호 0건일 때만 모델 판단으로 찾는다. 주차된 이슈는 다시 선택하지 않는다.
   - `memory/gotchas/` 폴더가 있으면 먼저 확인 — 같은 실수 반복 방지
   - `memory/learned/` 폴더가 있으면 참조 — 성공 패턴 활용
   - lint/typecheck/테스트가 빨간 상태면 "할 게 없음"을 선언하지 않는다(드라이 판정도 도구로)
2. **Fix**: 이슈를 해결하는 최소 변경을 적용한다.
3. **Verify**: 테스트/체크로 검증한다. **수정이 통합 경계를 건드렸으면 게이트 전에 경계면 교차비교 1회**(웹: API 응답 shape↔훅 타입·경로↔href·엔드포인트↔훅 1:1 / 비웹: 해당 경계 / 없으면 skip — "빌드 통과≠정상", 양쪽 동시 읽기). 실패 시 최대 3회 재시도. 3회 실패 → 능력 1회 상향(추론 effort 또는 더 강한 모델; 둘 다 불가하면 실패 출력을 증거로 접근 1회 재설계) → 그래도 실패 → 사유를 적어 주차(Park)하고 다음 이슈로. 같은 방식 재시도만으로 주차 금지(에스컬레이션은 이슈당 1회).
