...
- `started_at`으로 2시간 stale guard (Claude 훅만, Codex는 미구현)
- `active`: 참고용 메타데이터 (실제 활성 판단은 파일 존재 여부로)
- 루프 비활성화 = **상태 파일 제거**. 훅(`loop-stop.*`)이 종료 분기마다 자동 삭제하므로, AI는 직접 `rm`을 호출하지 않습니다. 사용자가 수동 중단하려면 `scripts/cancel-loop.{sh,ps1}` 사용.
