...
범위 판단 후, 각 gotcha에 대해 상위 계층(상시 가시 가드레일 / 관련 SKILL.md 내장) 승격 후보 여부를 판단해 파일 말미에 한 줄로 표기합니다: `**승격 판단**: {승격 금지 | 상시 가시 후보 | 스킬 내장 후보} — {근거 한 줄}`

| 유형 | 판단 |
|------|------|
| 유명/공개 함정 (공개 문서화 — 모델 내재 지식) | 승격 금지 — 정제 .md 보관만. T0 스크리닝 실측(2026-07-31): 모델이 실행 수준에서 이미 회피, 상시 노출은 예산 낭비 |
| 습관 재정의형 (알면서도 기본 습관이 함정 쪽으로 미는 것) | 상시 가시 후보 — 지식이 아니라 행동 교정이 필요 |
| 레포 고유형 (공개 문서 없는 시스템 규약·경로·마커) | 상시 가시 후보 — 모델이 알 수 없는 정보 |
| 경합 제약형 (다른 규칙이 함정 쪽으로 미는 구조) | 스킬 내장 후보 — 발동 작업에 결박 |
| 애매 | 승격 금지 (보수적 기본값 — 상시 가시 예산 보호) |

승격 실행은 이 에이전트의 범위 밖 — 후보 표기까지만. 근거: `docs/research/2026-07-31-q1-t0-screening-results.md`
