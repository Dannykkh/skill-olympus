---
name: gemini
description: Use when the user asks to run Gemini CLI for code review, plan review, or big context (>200k) processing. Ideal for comprehensive analysis requiring large context windows. Uses Gemini 3.1 Pro Preview by default for state-of-the-art reasoning and coding.
---
...
6. For interactive sessions with an initial prompt:
   ```bash
   gemini -m gemini-3.1-pro-preview -i "Review the authentication system" --approval-mode auto_edit
   ```
...
> Verify exact context window / pricing on the official model card — values may shift during preview phase.
...
gemini -m gemini-3.1-pro-preview --approval-mode default \
...
Requires recent Gemini CLI version for Gemini 3.x model support. Check version: `gemini --version` and update with `npm install -g @google/gemini-cli@latest` if needed.
