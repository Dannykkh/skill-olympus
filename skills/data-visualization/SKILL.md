...
- 원본: [anthropics/knowledge-work-plugins](https://github.com/anthropics/knowledge-work-plugins) `data/skills/data-visualization/SKILL.md` (Apache-2.0)
- 편입일: 2026-07-10 — 본문 무수정, frontmatter만 이 레포 규칙(호출 가능 + 한국어 트리거 설명)에 맞게 조정
- 원본 갱신 시 위 경로에서 재편입
...
| 파일 | 역할 |
|------|------|
| `skills/data-visualization/SKILL.md` | 차트 선택 가이드 + Python 시각화 패턴 + 디자인 원칙 + 접근성 체크리스트 |

## 연관 스킬

| 스킬 | 역할 분담 |
|------|----------|
| design-plan (아프로디테) / frontend-design | 웹 대시보드의 시각 품질(토큰·색·타이포)은 그쪽, "어떤 차트를 쓸지" 판단은 이 스킬 |
| mermaid-diagrams | 소프트웨어 구조 다이어그램(플로우차트·ERD·시퀀스) — 통계 그래프 아님 |
| ppt-generator | PPT에 차트 삽입 시 이 스킬의 선택 가이드를 먼저 적용 |
