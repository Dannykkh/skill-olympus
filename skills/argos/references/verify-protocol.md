...
> **native-first**: 이 중 spec과 무관한 일반 코드 품질(버그·타입·중복)은 네이티브 리뷰 엔진
> (Claude `code-review` / Codex `codex review`) 가용 시 그 결과를 병합하고 자체 스캔을 생략한다
...
API 관련 최종 판정에는 `SKILL.md`의 Phase 7-3a 게이트를 우선 적용한다. 관련 수정 후에는 해당
직접 요청과 저장/부수 효과 검증도 재실행한다. 아래 표의 모든 실패 해소는 필수 `NOT RUN` 해소까지
포함한다. 미실행이 남으면 최대 CONDITIONAL, 우회가 남으면 해당 게이트 FAIL 및 최종 PASS 금지다.
