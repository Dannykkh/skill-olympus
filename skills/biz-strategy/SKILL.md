...
`$ARGUMENTS`가 없으면 현재 CLI의 질문 방식으로 확인:
...
먼저 서로 뚜렷이 구별되는 비즈니스 모델 후보 2~3개를 생성하고, 각 후보를 수요(demand)·해자(moat)·단위 경제학(unit-economics) 기준으로 1~5점 채점합니다(각 점수마다 한 줄 근거). 점수표로 최적 후보를 선택한 뒤 BMC를 작성합니다.

> 채점 없이 후보만 나열 금지. 후보를 만들었으면 반드시 위 3개 기준으로 점수화하고, 선택 판단을 사용자에게 떠넘기지 않습니다.

선택된 후보에 대해 BMC 9블록(핵심 파트너~수익원)과 Lean Canvas(초기 스타트업용)를 작성합니다.
출력: 후보 점수표 + 텍스트 캔버스 + Mermaid 다이어그램
...
**근거 가드(필수):** 각 수치에 출처(보고서명·연도/검색 링크)를 병기합니다. 출처를 확인하지 못한 값은 단정하지 말고 `[확인 필요]`로 표기하고, 가정에서 도출한 값은 "추정"임과 산출 근거(가정·계산식)를 함께 명시합니다. 출처 없는 숫자를 사실처럼 제시하지 않습니다.
...
hermes 산출물 끝에 다음 섹션을 **반드시** 추가합니다. 이는 zephermine Step 8(사전 v1 자동 생성)에서 시드로 활용됩니다.

```markdown
## 핵심 도메인 용어 시드

> hermes 분석에서 등장한 도메인 용어. zephermine이 이 목록을 도메인사전 v1 시드로 활용합니다.

| 영문 식별자 (제안) | 한글 표기 | 정의 | 출처 영역 |
|------------------|-----------|------|----------|
| Customer | 고객 | 핵심 가치 제안의 수혜자 | 영역 1 (BMC) |
| Subscription | 구독 | 월/연 단위 정기 결제 모델 | 영역 2 (수익) |
| ARPU | 사용자당 평균 매출 | 월 매출 ÷ 활성 사용자 | 영역 5 (지표) |
| ... | ... | ... | ... |
```

**작성 규칙:**
- BMC/시장분석/지표에서 등장한 명사 위주 (5~15개)
- 영문 식별자는 표준 비즈니스 용어 우선 (CAC, LTV, ARPU, NPS 등)
- 한글 표기는 일반 통용 표기 (외래어 표기법 준수)
- 정의는 한 줄
- 도메인사전이 이미 있으면 (`docs/domain-dictionary.md`) → 충돌 시 표시. 없으면 → zephermine이 시드로 채택

> 이 섹션은 hermes의 사업 분석과 zephermine의 도메인사전 사이 **가교** 역할을 합니다.
