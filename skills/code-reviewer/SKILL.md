...
## API·입력 처리 변경의 필수 검토 (모든 리뷰 경로)

API·인증/권한·입력 스키마·데이터 바인딩·업무 상태·공통 검증기 변경이면
[API 직접 호출·서버 검증 계약](references/security-audit.md#api-직접-호출서버-검증-계약)을
변경 범위에 적용합니다. 경로 A/B/C, 소규모 diff, specialist 생략 여부와 무관합니다.
파일명 감지만으로 제외하지 말고 실제 호출 경로를 확인합니다. 네이티브 리뷰 결과의 누락을 이 계약으로
보강하고 중복 발견은 합칩니다. 위임 프롬프트에도 해당 계약을 전달하며 메인이 커버리지를 확인합니다.
실제 실행 근거가 없는 부분은 `UNVERIFIED`/`NOT RUN`으로 보고합니다.
