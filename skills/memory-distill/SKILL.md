...
| 트리거 | 빈도 | 깊이 | 분석 모델 | 역할 |
|--------|------|------|----------|------|
| Stop 훅 | 매 응답 | jsonl append | (모델 호출 X) | 관찰 수집 |
| 임계값(50) gotcha-analyzer | 가끔 | 가벼운 후보 추출 | **메인 세션 모델** (opus/gpt-5.5/3.1-pro) | 안전망 |
| **`/memory-distill`** | **사용자 의지** | **풀 정제 + rebuild** | **메인 세션 모델** | **주 정제** |
| 핸드오프 자동 추출 | 컨텍스트 위기 시 | 풀 정제 + 통합 | 메인 세션 모델 | 세션 경계 정제 |

분석 모델 정책:
- 모든 분석 트리거가 **호출자(메인 세션) 모델 상속** — frontmatter에 `model:` 미지정
- Anthropic Dreaming(`model: claude-opus-4-7`)과 동등한 분석 품질을 얻기 위한 설계
- 비용 vs 품질 트레이드오프: 임계값 50으로 격하되어 빈도가 낮으므로 비용 부담 제한적, 분석 품질 ↑로 노이즈 .md 감소 → 사용자 검토 비용 ↓
