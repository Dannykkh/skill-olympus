...
description: 프로젝트의 모든 verify 스킬을 순차 실행하여 통합 검증 보고서를 생성합니다. /verify-implementation으로 실행.
---
...
이슈가 발견된 경우 구조화된 질문 도구가 있으면 사용하고, 없으면 동일한 선택지를 일반 대화로 제시하고 사용자 응답을 받습니다:
...
각 이슈마다 수정 내용을 보여주고 구조화된 질문 도구 또는 일반 대화로 승인 여부를 확인합니다.
