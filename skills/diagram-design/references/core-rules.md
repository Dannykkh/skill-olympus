<!--
  VENDORED REFERENCE — 이 파일은 스킬 진입점이 아닙니다.
  원본: cathrynlavery/diagram-design skills/diagram-design/SKILL.md (v2.6, MIT)
  Pinned commit: 648c2a597839301e06df1e7434a08bde9f42eed3
  이 모듈의 진입점과 경계는 상위 SKILL.md가 소유합니다. §0(스타일 가이드 게이트)은
  상위 SKILL.md의 DESIGN.md 토큰 규칙으로 대체됩니다(인터랙티브 질문 금지 파이프라인 호환).
  references/ 중 이 저장소에 vendoring되지 않은 파일(onboarding.md, profiles.md,
  semantic-patterns.md, 미포함 type-*.md, assets/example-*.html)은 upstream에서만 존재합니다.
-->
