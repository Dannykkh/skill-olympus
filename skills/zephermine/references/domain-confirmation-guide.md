...
기본은 일반 텍스트 번호 목록입니다. 구조화 질문 도구가 다중 선택을 명시 지원할 때만 한 화면 선택형으로 묶고, 항목이 많으면 2-3개 질문 또는 그룹 단위로 나누어 진행합니다. **구조화 도구를 쓸 땐 한 호출당 `options`는 2~4개만** (`AskUserQuestion`은 옵션 4개 초과 시 `Invalid tool parameters` — 5개 이상이면 일반 텍스트 번호 목록으로 받거나 4개씩 분할). `header`도 12자 이내. `Invalid tool parameters`가 한 번이라도 나면 같은 payload를 재시도하지 말고 일반 텍스트로 전환합니다.
...
> 구조화 도구를 쓸 땐 한 호출당 options 2~4개만 (5개 이상이면 🔴/🟡/🟢 그룹별로 나누어 질문 — 4개 초과 시 `Invalid tool parameters`). 구조화 도구가 불안정하면 `1, 3, 5 채택`처럼 번호 답변을 받습니다.
