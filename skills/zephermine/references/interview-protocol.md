...
질문 방향:
- **역할 목록**: "이 서비스에 로그인하는 사람은 어떤 종류가 있나요? (예: 관리자, 직원, 일반 회원, 거래처 담당자)"
- **역할별 업무**: "{역할}은 여기서 주로 무슨 일을 하나요? 그 역할만 할 수 있어야 하는 일은 뭔가요?"
- **화면 분리**: "{역할}은 다른 역할과 **같은 화면을 권한만 다르게** 쓰나요, 아니면 **별도 화면이나 앱**이 필요한가요?"
- **금지 행동**: "{역할}이 절대 하면 안 되는 일이 있나요? (타인 데이터 조회, 금액 수정, 삭제, 대량 내보내기 등)"
