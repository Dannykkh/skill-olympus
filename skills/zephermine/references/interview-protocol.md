...
- 이해관계자 중 **시스템에 로그인하는 주체만** 골라내고, 인터뷰에 안 나온 시스템 전용 주체(비로그인 방문자, 배치/서비스 계정, 슈퍼관리자)를 되물어 확인. 이 결과가 Step 8 `spec.md`의 시스템 역할 표(Role Inventory)가 되며 이후 DB·API·도면·운영 시나리오의 역할명 정본이 됨
