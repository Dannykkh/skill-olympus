...
인터뷰 CPS Phase S-1(차별화 + 디자인 비전)에서 수집한 답변을 **구현자용 디자인 규칙 문서**로 변환합니다.

## 생성 조건

- UI/프론트엔드가 포함된 프로젝트에서만 생성
- 인터뷰 Phase S의 S-1(차별화 + 디자인 비전) 답변이 존재해야 함
- CLI, 라이브러리, 백엔드 API 전용 프로젝트는 건너뜀

## 입력

- `interview.md` — Phase S, S-1 답변 (차별화, 톤/무드, 벤치마킹, 색상, 레이아웃, 아트 디렉션, 안티 패턴)
- `interview.md` — Gate 3 Result (솔루션 방향 요약 — 차별화, 디자인 톤, 기술 방향)
...
> **참고**: 이 문서는 젭마인의 **간이 디자인 시스템**입니다.
> 더 정교한 디자인이 필요하면 `/aphrodite`(아프로디테)를 실행하세요.
> 아프로디테는 161개 팔레트 DB, 73개 폰트 페어링 DB, 84개 스타일 DB를 활용하여
> 이 문서를 기반으로 더 풍부한 디자인 시스템을 생성합니다.
...
> 이 문서는 젭마인 인터뷰 Phase S-1에서 수집한 디자인 비전을 기반으로 생성되었습니다.
> 구현 시 이 문서를 참조하여 일관된 디자인을 유지하세요.
> 더 정교한 디자인이 필요하면 `/aphrodite`를 실행하세요.
...
- {절대 하지 말아야 할 디자인 패턴}
- {AI Slop 블랙리스트에서 해당 프로젝트에 적용되는 항목}
...
- **더 정교한 디자인이 필요하면 `/aphrodite` 실행** — DB 기반 팔레트/폰트/스타일 매칭
```
...
6. **AI Slop 방지**: 과사용 폰트, 보라 그라데이션, 3열 카드 등 제네릭 패턴 피하기
