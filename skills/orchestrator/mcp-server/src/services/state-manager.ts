...
export class StateManager {
  ...
  private workerProvider?: AIProvider;
  ...
  private initTables(): void {
    ...
    const workerColumns = this.db.prepare('PRAGMA table_info(workers)').all() as { name: string }[];
  ...
  private migrateFromJson(stateDir: string): void {
    const jsonPath = path.join(stateDir, 'state.json');
    if (!fs.existsSync(jsonPath)) return;

    // 이미 마이그레이션 완료 여부 확인
    const taskCount = this.db.prepare('SELECT COUNT(*) as cnt FROM tasks').get() as { cnt: number };
    if (taskCount.cnt > 0) return; // 이미 데이터 있으면 건너뜀
    ...
    try {
      ...
      const migrate = this.db.transaction(() => {
        for (const t of state.tasks || []) {
          insertTask.run(
            t.id, t.prompt, t.status, t.owner || null,
            JSON.stringify(t.dependsOn || []),
            JSON.stringify(t.scope || []),
            t.priority || 1, t.aiProvider || null,
            t.createdAt, t.startedAt || null, t.completedAt || null,
        ...
        const migrateLog = this.db.transaction(() => {
          ...
          for (const line of lines) {
            try {
              ...
              insertLog.run(
                entry.timestamp, entry.workerId, entry.taskId || null,
  ...
  private isPathOverlap(pathA: string, pathB: string): boolean {
  ...
  public getWorkers(): WorkerInfo[] {
    const rows = this.db.prepare('SELECT * FROM workers').all() as Record<string, unknown>[];
    return rows.map(r => ({
      id: r.id as string,
      aiProvider: (r.ai_provider as AIProvider) || undefined,
  ...
  public getProgress(): ProgressInfo {
    const rows = this.db.prepare('SELECT id, status, owner, started_at, depends_on FROM tasks').all() as Record<string, unknown>[];
  ...
  public getAvailableTasks(): {
    ...
    availableTasks: {
      id: string; prompt: string; priority: number;
      scope?: string[]; aiProvider?: AIProvider;
    ...
    const allTasks = this.db.prepare('SELECT * FROM tasks').all() as Record<string, unknown>[];
  ...
  public claimTask(taskId: string): { success: boolean; message: string; task?: Task } {
    ...
    const claim = this.db.transaction(() => {
      ...
      return this.db.prepare('SELECT * FROM tasks WHERE id = ?').get(taskId) as Record<string, unknown>;
    });

    const claimedRow = claim();
    if (!claimedRow) {
      const row = this.db.prepare('SELECT * FROM tasks WHERE id = ?').get(taskId) as Record<string, unknown> | undefined;
      if (!row) return { success: false, message: `Task '${taskId}' not found` };
      ...
      const completedIds = new Set(
        (this.db.prepare("SELECT id FROM tasks WHERE status = 'completed'").all() as { id: string }[]).map(r => r.id)
      );
  ...
  public completeTask(
    ...
    // 의존성 해소된 태스크 찾기
    const allTasks = this.db.prepare('SELECT id, status, depends_on FROM tasks').all() as Record<string, unknown>[];
    const completedIds = new Set(
      allTasks.filter(t => (t.status as string) === 'completed').map(t => t.id as string)
    );
    completedIds.add(taskId);

    const unlockedDependents = allTasks
      .filter(t => (t.status as string) === 'pending')
      .filter(t => {
  ...
  public failTask(taskId: string, error: string): { success: boolean; message: string } {
    const row = this.db.prepare('SELECT owner FROM tasks WHERE id = ?').get(taskId) as { owner: string } | undefined;
    if (!row) return { success: false, message: `Task '${taskId}' not found` };
  ...
  public getActivityLog(query: ActivityQuery = {}): { entries: ActivityEntry[]; total: number } {
    ...
    if (query.taskId) { sql += ' AND task_id = ?'; params.push(query.taskId); }
    if (query.workerId) { sql += ' AND worker_id = ?'; params.push(query.workerId); }
    if (query.type) { sql += ' AND type = ?'; params.push(query.type); }
    if (query.since) { sql += ' AND timestamp >= ?'; params.push(query.since); }

    sql += ' ORDER BY rowid ASC';

    const totalRow = this.db.prepare('SELECT COUNT(*) as cnt FROM activity_log').get() as { cnt: number };

    if (query.limit && query.limit > 0) {
      // 최신 N건 → 전체에서 오프셋 계산
      const countSql = sql.replace('SELECT *', 'SELECT COUNT(*) as cnt');
      const filtered = this.db.prepare(countSql).get(...params) as { cnt: number };
  ...
  public deleteTask(taskId: string): { success: boolean; message: string } {
    // 의존성 확인
    const allTasks = this.db.prepare('SELECT id, depends_on FROM tasks').all() as Record<string, unknown>[];
  ...
  public isAllTasksCompleted(): boolean {
    const total = this.db.prepare('SELECT COUNT(*) as cnt FROM tasks').get() as { cnt: number };
    if (total.cnt === 0) return false;

    const remaining = this.db.prepare(
      "SELECT COUNT(*) as cnt FROM tasks WHERE status NOT IN ('completed', 'failed')"
    ).get() as { cnt: number };
