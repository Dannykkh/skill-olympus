...
async function spawnWorkers(count: number, autoTerminate: boolean, providers?: AIProvider[]): Promise<{
  success: boolean;
  message: string;
  ...
  const spawnedWorkers: { id: string; status: string; provider: string }[] = [];
  const errors: string[] = [];
  ...
  try {
    ...
    switch (name) {
      ...
      case 'orchestrator_spawn_workers': {
        ...
        result = await spawnWorkers(parsed.count, parsed.auto_terminate, parsed.providers as AIProvider[] | undefined);
        break;
      }
      ...
      case 'orchestrator_check_worker_logs': {
        const logDir = path.join(PROJECT_ROOT, '.orchestrator', 'logs');
        const workerId = (args as { worker_id?: string }).worker_id;
        ...
        const workerLogs: { workerId: string; status: string; lastLines: string[]; logFile: string }[] = [];
        for (const file of logFiles) {
          ...
          let status = 'unknown';
          if (lines.some(l => l.includes('CLI_STARTED'))) status = 'running';
          else if (lines.some(l => l.includes('ERROR') || l.includes('FATAL'))) status = 'error';
          else if (lines.some(l => l.includes('spawn initiated'))) status = 'spawned';
          if (lines.some(l => l.includes('finished successfully'))) status = 'completed';
