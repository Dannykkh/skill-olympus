...
# prompt 필드 추출 (jq 없이 grep/sed 사용)
PROMPT=$(echo "$INPUT" | grep -o '"prompt"[[:space:]]*:[[:space:]]*"[^"]*"' | sed 's/^"prompt"[[:space:]]*:[[:space:]]*"//;s/"$//')

# pmworker로 시작하는지 확인 (대소문자 무시)
if ! echo "$PROMPT" | grep -qiE '^\s*pmworker'; then
    exit 0
fi
