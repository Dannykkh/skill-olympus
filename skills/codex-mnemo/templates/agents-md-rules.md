...
- Codex의 읽기 전용 탐색은 `explorer`, 파일 생성·수정·명령 실행은 `worker`에 맡긴다. 읽기 전용 작업자에게 쓰기 작업을 주지 않는다.
...
이 문서의 `<CODEX_HOME>`은 `CODEX_HOME` 환경 변수 값이며, 미설정 시 `~/.codex`입니다.
`<CODEX_HOME>/SKILLS-CATALOG.md`는 기본 active 스킬과 저장소에만 남은 source-only 스킬을 함께 목록화합니다. `<CODEX_HOME>/AGENTS-CATALOG.md`는 명시 opt-in으로 설치된 사용자 정의 에이전트의 상태표이며 기본값은 0개입니다.

**스킬 조회 순서:**
1. Codex 전역 `<CODEX_HOME>/SKILLS-CATALOG.md`
2. 현재 프로젝트에 실제 파일이 있을 때만 `./SKILLS-CATALOG.md`
3. 둘 다 없으면 오류로 멈추지 말고 `<CODEX_HOME>/skills/*/SKILL.md`를 직접 탐색
...
- 사용자 요청에 `workpm`, `workpm-mcp`, `/daedalus`, `다이달로스`, `pmworker`가 있으면 즉시 `workpm` skill을 읽고 PM 모드로 들어갑니다. MCP 대규모 모드는 workpm이 카탈로그의 source-only `orchestrator` 모듈을 직접 읽어 사용합니다.
- 사용자 요청에 `agent-team`, `/agent-team`, `/poseidon`, `포세이돈`, `poseidon`이 있으면 즉시 `agent-team-codex` skill을 읽고 팀 리드 모드로 들어갑니다.
- 사용자 요청에 `/aphrodite`, `/hermes`, `/athena`, `/mnemo` 또는 대응 한국어 호출명이 있으면 해당 스킬(`design-plan`, `biz-strategy`, `ceo`, `codex-mnemo`)을 즉시 읽고 실행합니다.
...
3. **SessionStart**: 세션 시작 이벤트가 없어 `reconcile-conversations` 자동 호출 불가. 수동 호출만:
   ```bash
   python "${CODEX_HOME:-$HOME/.codex}/scripts/reconcile_codex_conversations.py" --days 7
   ```
...
Codex의 conversation은 `<CODEX_HOME>/sessions/.../rollout-*.jsonl`이 source of truth입니다.
...
**⚠️ JSONL 직접 읽기 절대 금지:**
- `<CODEX_HOME>/sessions/**/rollout-*.jsonl`, `~/.claude/projects/**/*.jsonl`은 mnemo 내부 백업입니다 (reconcile 전용).
- 검색 대상은 **오직 `conversations/*.md`** 입니다. Read 도구로 jsonl을 직접 열지 마세요.
- `conversations/`에서 못 찾아도 jsonl fallback 금지. 다음 절차를 따르세요:
  1. `python "${CODEX_HOME:-$HOME/.codex}/scripts/reconcile_codex_conversations.py" --days 7` 으로 누락 turn backfill
  2. backfill 후 `conversations/` 재검색
...
> `source`는 이 항목을 작성한 CLI를 표시합니다 (claude, codex, antigravity, grok).

**⚠️ 작성 시 절대 금지:**
- `source:` 필드에 사용자 prompt, 분석 텍스트, 코드 블록 넣지 말 것 — 정확히 `claude`/`codex`/`antigravity`/`grok` **한 단어**만. 원본 텍스트는 본문 `-` 항목에 작성
