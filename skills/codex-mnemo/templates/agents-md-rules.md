...
**⚠️ JSONL 직접 읽기 절대 금지:**
- `~/.codex/sessions/**/rollout-*.jsonl`, `~/.claude/projects/**/*.jsonl`은 mnemo 내부 백업입니다 (reconcile 전용).
- 검색 대상은 **오직 `conversations/*.md`** 입니다. Read 도구로 jsonl을 직접 열지 마세요.
- `conversations/`에서 못 찾아도 jsonl fallback 금지. 다음 절차를 따르세요:
  1. `python ~/.codex/scripts/reconcile_codex_conversations.py --days 7` 으로 누락 turn backfill
