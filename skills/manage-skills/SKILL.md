---
name: manage-skills
description: 세션 변경사항을 분석하여 검증 스킬(verify-*) 생성/업데이트 및 프로젝트 지침 파일(기본 `AGENTS.md`)을 관리합니다. /manage-skills로 실행.
---
...
등록된 스킬이 1개 이상인 경우, 각 스킬의 `skills/verify-<name>/SKILL.md`를 읽고 다음에서 추가 파일 경로 패턴을 추출합니다:
...
구조화된 질문 도구가 있으면 사용하고, 없으면 일반 대화로 확인합니다:
- 어떤 기존 스킬을 업데이트할지
- 제안된 새 스킬을 생성할지
- 전체 건너뛰기 옵션
...
2. **사용자에게 스킬 이름 확인** — 구조화된 질문 도구가 있으면 사용하고, 없으면 일반 대화로 확인합니다:

   **이름 규칙:**
   - 이름은 반드시 `verify-`로 시작해야 합니다 (예: `verify-auth`, `verify-api`, `verify-caching`)
   - 사용자가 `verify-` 접두사 없이 이름을 제공하면 자동으로 앞에 추가하고 알립니다
   - kebab-case를 사용합니다 (예: `verify-error-handling`)

3. **생성** — `skills/verify-<name>/SKILL.md`를 다음 템플릿에 따라 생성합니다:
...
4. **연관 스킬 파일 업데이트** — 새 스킬 생성 후 반드시 아래 3개 파일을 업데이트합니다:
   ...
   **4c. 프로젝트 지침 파일 업데이트:**
   - 기본 대상은 `AGENTS.md`입니다
   - 별도 사용자 문맥 파일이 유지되는 프로젝트라면 필요한 항목만 선택적으로 동기화합니다
...
### Step 7.5: 스코어 기반 hill-climbing (선택, 권장)

새로 생성한 verify-* 스킬은 일회성 검증 대신 채점 루프로 다듬는 것을 권장합니다:

1. 위 **품질 기준**(실제 파일 경로, 작동하는 탐지 명령어, PASS/FAIL 기준, 현실적 예외, 일관된 형식)을 3-6개 예/아니오 체크리스트로 변환합니다.
2. 새 verify-* SKILL.md에 대해 그 체크리스트로 `/autoresearch`를 실행합니다. 한 번에 하나씩 변경하며 개선은 유지하고 악화는 되돌려, 일회성 검증을 keep-or-revert 루프로 전환합니다.
3. 사소한 변경만 있는 스킬은 Step 7 검증으로 충분하므로 건너뜁니다.
...
| File | Purpose |
|------|---------|
| `skills/verify-implementation/SKILL.md` | 통합 검증 스킬 (이 스킬이 실행 대상 목록을 관리) |
| `skills/manage-skills/SKILL.md` | 이 파일 자체 (등록된 검증 스킬 목록을 관리) |
...
6. **AGENTS.md/보조 지침 파일 자체** — 지침 문서 변경은 문서 업데이트이며, 검증이 필요한 코드 패턴이 아님. 기본 정본은 `AGENTS.md`로 봅니다.
