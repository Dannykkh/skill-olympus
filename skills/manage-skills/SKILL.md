...
`AskUserQuestion`이 있으면 사용하고, 없으면 일반 대화로 확인합니다:
- 어떤 기존 스킬을 업데이트할지
- 제안된 새 스킬을 생성할지
- 전체 건너뛰기 옵션
...
2. **사용자에게 스킬 이름 확인** — `AskUserQuestion`이 있으면 사용하고, 없으면 일반 대화로 확인합니다:
...
4. **연관 스킬 파일 업데이트** — 새 스킬 생성 후 반드시 아래 3개 파일을 업데이트합니다:
   ...
   **4c. 프로젝트 지침 파일 업데이트:**
   ...
   - 별도 `CLAUDE.md` 복사본이 있으면 같은 내용으로 동기화합니다
...
6. **AGENTS.md/CLAUDE.md 자체** — 지침 문서 변경은 문서 업데이트이며, 검증이 필요한 코드 패턴이 아님
