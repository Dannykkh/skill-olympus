...
루트 안의 파일은 루트 기준 상대경로(슬래시 구분)로, 루트 밖의 파일은 절대경로 그대로 적는다.
절대경로를 남기면 프로젝트를 옮기거나 다른 컴퓨터에서 열었을 때 기록이 옛 위치를 가리킨다.
도구 관찰 훅은 `file_path`·`notebook_path`·`path` 필드에만 적용하고 명령·본문은 손대지 않는다.
핸드오프의 `Project:`는 폴더 이름만 담으며, 루트 판별은 항상 핸드오프 파일 위치로 한다.
