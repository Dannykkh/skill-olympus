...
- Claude의 읽기 전용 탐색은 `Explore`, 파일 생성·수정·명령 실행은 `general-purpose` 또는 이름 있는 teammate에 맡긴다. 읽기 전용 작업자에게 쓰기 작업을 주지 않는다.
...
- **DESIGN.md 먼저 읽기**: 프로젝트 루트에 `DESIGN.md`(YAML 토큰 + 산문 = 디자인 정본)가 있으면 UI 작업 전 **반드시 먼저 읽고** 그 토큰(색·타이포·간격·라운드)을 따른다 — "부를 때마다 색/폰트가 달라지는" 드리프트 방지. 코드에 CLAUDE.md가 있다면 디자인엔 DESIGN.md. 없고 본격 작업이면 `/aphrodite`가 생성·lint.
...
`~/.claude/SKILLS-CATALOG.md`는 기본 active 스킬과 저장소에만 남은 source-only 스킬을 함께 목록화합니다. `~/.claude/AGENTS-CATALOG.md`는 명시 opt-in으로 설치된 사용자 정의 에이전트의 상태표이며 기본값은 0개입니다.

**스킬 조회 순서:**
1. Claude 전역 `~/.claude/SKILLS-CATALOG.md`
2. 현재 프로젝트에 실제 파일이 있을 때만 `./SKILLS-CATALOG.md`
3. 둘 다 없으면 오류로 멈추지 말고 `~/.claude/skills/*/SKILL.md`를 직접 탐색
...
- 일치하는 skill이 있으면 해당 skill을 명시적으로 사용하세요.
...
**⚠️ JSONL 직접 읽기 절대 금지:**
- `~/.claude/projects/**/*.jsonl`, `~/.codex/sessions/**/rollout-*.jsonl`은 mnemo 내부 백업입니다 (reconcile 전용).
- 검색 대상은 **오직 `conversations/*.md`** 입니다. Read 도구로 jsonl을 직접 열지 마세요.
- `conversations/`에서 못 찾아도 jsonl fallback 금지. 다음 절차를 따르세요:
  1. `python skills/mnemo/scripts/reconcile_conversations.py --all` 으로 누락 turn backfill
...
- 대화 저장 훅이 첫 저장 턴에서 기본 scaffold를 자동 생성하지만, 그 전에 직접 써야 하면 즉시 만들어 사용합니다.
...
3. **핸드오프 파일 생성** — `docs/handoffs/YYYY-MM-DD-HHMMSS-{slug}.md`에 아래 형식으로 작성:
