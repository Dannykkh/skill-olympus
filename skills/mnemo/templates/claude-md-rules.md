...
"X 구현/추가/만들어" 요청엔 코드 쓰기 전에 기존부터 조회한다 — **`codemap/index.md` 먼저**(코드 위치, Code Map 규칙: 코드맵→카테고리, 일반 grep은 나중) → README/docs + `docs/handoffs/` 최근 Feature·Menu/Screen Map(세션 기록) → 못 잡으면 핵심어 grep(보조). 분류 — **신규**(없음)면 구현, **개선·중복**(이미 있음)이면 재구현 말고 "이미 {위치}에 {경계}까지 있음 — 개선이냐 새로냐?" 확인 후 진행. 사소한 편집·단일 버그픽스·명시적 "새로 만들어"·이미 zephermine 안이면 제외(과적용 금지).
