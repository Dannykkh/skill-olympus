...
- **폰트 실제 로드(검증)**: 고른 폰트는 `@import`/`<link>`로 **실제 로드**해야 함 — 이름만 `font-family`에 쓰면 시스템 폰트로 조용히 폴백됨(육안 구분 어려움 → `document.fonts.check('700 16px "X"')`로 확인). **한글 포함 UI는 한글 웹폰트(Pretendard 등)를 폰트 스택에 함께 로드** — 라틴 페어링(Space Grotesk·DM Sans 등)엔 한글 글리프가 없어 한글이 시스템 폴백된다. 쓰는 weight만 import(미사용 weight 로드 금지).
