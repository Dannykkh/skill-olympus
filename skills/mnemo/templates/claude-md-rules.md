...
AI 기본값은 "인터넷의 평균"(Inter·보라/파랑 그라데이션 hero·"Build the future"류 카피·균일 16px 라운드 카드 그리드)으로 수렴합니다. UI를 생성/재디자인할 때는 그 평균에서 의도적으로 벗어나세요. 본격 작업은 `/frontend-design`(또는 `/aphrodite`)이 처리하고, 이건 항상 적용되는 압축본입니다.
...
**조회 순서:**
1. Claude 전역 카탈로그: `~/.claude/SKILLS-CATALOG.md`, `~/.claude/AGENTS-CATALOG.md`
2. 현재 프로젝트에 실제 파일이 있을 때만: `./SKILLS-CATALOG.md`, `./AGENTS-CATALOG.md`
3. 둘 다 없으면 오류로 멈추지 말고 `~/.claude/skills/*/SKILL.md`, `~/.claude/agents/*.md`를 직접 탐색
