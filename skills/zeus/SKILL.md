...
Phase 2.5: Verification (argos) ─ 설계 대비 준공검사
    │
    ▼
Phase 2.7: Docker Setup (docker-deploy) ─ Docker 환경 구성 + 포트 충돌 해결
    │
    ▼
...
6. **모든 Phase 강제 실행** — Phase 0~4 모두 최소 1회 실행 시도 필수. "건너뜀"은 물리적 불가(Docker 미설치 등)일 때만 허용하며, 그 경우에도 폴백 경로를 실행

### 실행 보장 게이트 (Phase 4 시작 전 필수 — 이 게이트를 건너뛰면 안 됩니다)

모든 Phase의 실행 증거가 있어야 Phase 4를 시작할 수 있습니다:

- Phase 1: `plan.md` 존재
- Phase 2: `docs/zeus/zeus-log.md`에 workpm task 생성/worker 실행 기록
- Phase 2.5: `docs/zeus/zeus-log.md`에 argos 실행 기록 (최소 정적 분석)
- Phase 2.7: `docs/zeus/zeus-log.md`에 Docker/dev-server 시도 기록
- Phase 3: QA 결과 파일 또는 `docs/zeus/zeus-log.md`에 qpassenger 실행 기록

- **위 중 하나라도 없으면 Phase 4를 시작하지 말고, 누락된 phase를 먼저 실행**
...
```
Phase 1 완료
    │  plan.md + sections/ 생성됨
    ▼
1. plan.md → .claude/plans/zeus-{projectName}.md 로 복사
...
- `plan.md` 미생성 시: Phase 0/인터뷰 산출물 기반 최소 plan을 생성 후 즉시 workpm 실행
...
### 폴백 조건 (Phase 2.7도 skip 금지 — 최소 서버 실행 시도 필수)
...
3. ✅ `docs/zeus/zeus-log.md`에 argos 실행 기록? → 없으면 Phase 2.5 실행
4. ✅ `docs/zeus/zeus-log.md`에 서버 실행 시도 기록? → 없으면 Phase 2.7 실행
5. ✅ qpassenger 실행 기록(QA 결과 파일 또는 로그)? → 없으면 Phase 3 실행
...
## Phase 2.5: Verification (argos) — ✅ / ⚠️
- 검증 항목: N개, 통과: N, 미통과: N

## Phase 2.7: Docker Setup — ✅ / ⚠️ 폴백
