...
> **항목 2 배점 제외 근거**: Google이 AI 검색 최적화 안내에서 `llms.txt`를 실효성 없는
> 항목으로 분류했고, 독립된 두 구현(`AgriciDaniel/claude-seo`, `Hainrixz/claude-seo-ai`)이
> 각각 "myth"와 "weight 0"으로 동일하게 처리합니다. 존재 여부는 리포트에 적되
> **가감점하지 않습니다.** 없다고 해서 "만드세요"라고 권하지 마세요.
> 상세 → [scoring-model.md](scoring-model.md#llmstxt--가중치-0-보고만-함)
...
**점수 축: AI 가시성 (가중치 15)**
**계측 근거**: `python scripts/fact_density.py <콘텐츠 경로> --json`의
`question_headings`, `answer_leads_over_40w`

| # | 점검 | 미달 시 등급 |
|---|------|------------|
| 1 | FAQ 콘텐츠에 `FAQPage` JSON-LD 스키마 적용 여부 | P2 — Q&A 구조 신호 (아래 참조) |
| 2 | 질문형 헤딩(`<h2>`, `<h3>`) 사용 여부 | P1 — AI가 Q&A 패턴을 인식 못함 |
| 3 | 간결한 답변 블록 존재 (헤딩 바로 아래 40단어 이내 요약) | P2 — AI 발췌율 2.7배 차이 |
| 4 | `HowTo` 스키마 적용 여부 (가이드/튜토리얼 페이지) | P3 — 리치 결과 축소됨 |
...
**점수 축: AI 가시성 (가중치 12)**
**계측 근거**: `python scripts/fact_density.py <콘텐츠 경로> --json`

| # | 점검 | 계측 필드 | 미달 시 등급 |
|---|------|----------|------------|
| 1 | 콘텐츠에 구체적 수치/통계가 포함되어 있는지 | `numeric_density_per_100w` | P1 — AI는 "주장+근거"가 있는 콘텐츠를 인용 |
| 2 | 출처/참조 표기 여부 (링크, 인용문) | `citation_markers` | P1 — AI가 신뢰할 수 있는 소스로 판단 |
| 3 | 저자 정보와 전문성 신호 (Author JSON-LD, 약력) | `authoritative_outbound_links` | P2 — E-E-A-T 신호 |
| 4 | 콘텐츠 첫 200단어가 핵심 질문에 직접 답변하는지 | `answer_leads_over_40w` | P1 — AI가 페이지 상단 우선 평가 |
| 5 | `llms.txt` 파일 제공 여부 | — | **배점 없음 — 보고만 함** (영역 8 항목 2 참조) |
| 5b | 근거 없는 과장 표현 사용 (역신호) | `superlative_or_vague_authority` | P2 — "최고의", "전문가들은" 등은 인용을 방해 |
