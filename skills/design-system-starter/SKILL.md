...
|----------|---------|---------------|
| **Color** | Primitive (50-950 scale) + Semantic (brand, text, bg, feedback) | WCAG 2.1 AA contrast ratios |
| **Typography** | Font family, size scale, weight, line-height | Base 16px, modular scale |
| **Spacing** | 4px or 8px base scale (0-24) + component-specific | Consistent multiplier |
| **Border Radius** | none → sm → base → md → lg → xl → 2xl → full | Match brand personality |
| **Shadow** | xs → sm → base → md → lg → xl | Elevation hierarchy |

> 상세 JSON 예시: [references/design-tokens.md](references/design-tokens.md) — 각 카테고리별 전체 토큰 값 포함
